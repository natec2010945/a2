...
void nonstring_tokenize_test(const char * label, ITERATOR begin, ITERATOR end, FN splitfn, auto delim) {
  using std::cout;
  const char indent[] = "      ";
...
int main_test(CONTAINER source) {
  using std::cout;
...
int main(int argc, char *argv[]) {
  if(argc > 1) {  // got command line arguments, handle
    ...
    std::cout << "GRABBING INPUT VALUES FROM STANDARD INPUT\n";
